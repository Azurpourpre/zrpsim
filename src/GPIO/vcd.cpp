VCDWriter::VCDWriter(const char* filename){
...
void VCDWriter::write_header(const char* timescale){
...
void VCDWriter::write_time(const char* timestamp){
    this->outfile << "#" << timestamp << std::endl;
}

void VCDWriter::write_value(VCDSignal* sig, const uint32_t val){
...
VCDSignal* VCDWriter::create_signal(const uint8_t size, std::string wanted_name){
    // Instantiate signal
